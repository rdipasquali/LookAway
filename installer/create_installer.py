#!/usr/bin/env python3
"""
Resource Embedder - Creates installer with actual application files.
This embeds the LookAway.exe and other necessary files into the installer.
"""
...
def get_app_files():
    ...
    if lookaway_exe.exists():
        ...
        with open(lookaway_exe, 'rb') as f:
            ...
            files_to_embed['LookAway.exe'] = encoded
...
def create_installer_with_files(output_file="windows_installer_with_files.py"):
    ...
    embedded_files_code += "        if 'LookAway.exe' in files and isinstance(files['LookAway.exe'], bytes):\n"
    embedded_files_code += "            files['LookAway.exe'] = gzip.decompress(files['LookAway.exe'])\n"
    embedded_files_code += "        \n"
...
def main():
    """Main execution."""
    print("=" * 60)
    print("Creating Installer with Embedded Files")
    print("=" * 60)
